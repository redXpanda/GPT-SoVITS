...
async def index():
    """内置 API 测试台页面，读取同目录下的 api_v2_official_test.html 返回喵～"""
    html_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "api_v2_official_test.html")
    if not os.path.exists(html_path):
        return HTMLResponse(
            content="<h1>测试页面缺失</h1><p>未找到 api_v2_official_test.html，请确认它与本脚本在同一目录喵～</p>",
            status_code=404,
        )
